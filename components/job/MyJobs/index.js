...
const MyJobs = ({ jobs, access_token }) => {
  ...
  const { loading, error, deleted, deleteJob, setDeleted, clearErrors } = useContext(JobContext)
  ...
  jobs && jobs.forEach((job) => {
    const { id, title, salary, education, experience } = job;
    data.push({
      id,
      title,
      salary,
      action: (
        <>
          ...
          <button className="btn btn-danger mx-1" onClick={() => handleDelete(job.id)}>
